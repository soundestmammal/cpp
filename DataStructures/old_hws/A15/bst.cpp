...
farmingdale::bstNode * copyTree(const farmingdale::bstNode * copyNode) {
	...
	try {
		...
		newNode = new farmingdale::bstNode(copyNode->key, copyNode->value);
	} catch (std::bad_alloc) {
		return 0;
	}
...
farmingdale::status farmingdale::bst::insert(std::string addMe, int payload) {
	bstNode* current = root;
	bstNode* trailCurrent = 0;
	while (0 != current && current->key != addMe) {
		trailCurrent = current;
		current = (addMe < current->key) ? current->leftChild : current->rightChild;
	}
	...
	bstNode * newNode;
	try {
		newNode = new bstNode(addMe, payload);
	}
	catch (std::bad_alloc) {
		return FAILURE;
	}
...
farmingdale::bstNode * findNodesParent(farmingdale::bstNode * root, farmingdale::bstNode *theNode) {
	...
	while (theNode != current) {
		trailCurrent = current;
		current = (current->key> theNode->key)?  current->leftChild : current->rightChild;
	}
	return trailCurrent;
}
...
void setCorrectChild(farmingdale::bstNode * theNodeToChange, farmingdale::bstNode *childNode, farmingdale::bstNode *newChild) {
	if (theNodeToChange->key > childNode->key) {
...
farmingdale::status farmingdale::bst::remove(bstNode* removeMe) {
	...
	case 2:
		...
		// store the data
		theKey = successor->key;
		theValue = successor->value;
...
farmingdale::bstNode * farmingdale::bst::search(std::string findMe) const {
	...
	while (0 != current && current->key != findMe) {
		if (findMe < current->key) {
			current = current->leftChild;
		} else {
...
farmingdale::status farmingdale::bst::query(std::string findMe, int& passMeBackPlease) {
	bstNode* theReturnedNode = search(findMe);
	if(NULL != theReturnedNode) {
		// WE found it!!!
		passMeBackPlease = theReturnedNode->value;
		return SUCCESS;
	}
	passMeBackPlease = -1;
	return FAILURE;
