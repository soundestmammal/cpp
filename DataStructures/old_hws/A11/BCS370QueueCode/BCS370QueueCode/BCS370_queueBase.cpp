...
farmingdale::statusCode farmingdale::queue::enqueue(std::string addMe) {
	...
	if(isFull()) {
		std::cout << "Changing the buffer size from " << bufferSize << " to " << bufferSize*2 << std::endl; // 16 -> 32
		// std::cout << "Buffer size is " << bufferSize << std::endl; // 16
		// bufferSize = bufferSize*2;
		// std::cout << "Buffer size is " << bufferSize << std::endl; // 32
		std::string * temp;
...
farmingdale::statusCode farmingdale::queue::peek(std::string &returnedElement) const {
	// if empty, return FAILURE
	if (isEmpty()) {
		std::cout << "This is into of the queue cpp dynamic... " << std::endl;
		return FAILURE;
	}
	// get the OLDEST item, put it in returnedElement
	returnedElement = data[oldestIndex];
