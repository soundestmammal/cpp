...
private:
	...
	std::string data[INITIAL_QUEUE_SIZE];			  // This should become a dynamic array

	int oldestIndex;		// index of the oldest element in the queue
	int nextInsertIndex;	// index of the first empty spot

	inline int queueCapacity() const { return INITIAL_QUEUE_SIZE; } // returns the current queue capacity-will change in dynamic
